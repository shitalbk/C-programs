...
public class OldBejeweled implements ActionListener {
    ...
    public OldBejeweled() {
        //Set up window
        JFrame frame = new JFrame("Bejeweled");//Inherit new Jframe
        frame.setLayout(new GridLayout(9, COL));//set Layout 
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//setClose operation 
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                pic = getPic(i, j);
                jewel[i][j].setActionCommand("(" + i + "," + j + "," + pic + ")");//(2,3,random num)
                jewel[i][j].addActionListener(this);//Action listener yo class ho set gar cha
        ...
        JPanel status = new JPanel();// jpanel
    ...
    public void eliminateRow(int x, int y, int num, int mode) {
        ...
        if (mode == 1) {
            ...
            for (int i = y; num >= 0; i--, num--) {
                jewel[x][i].setActionCommand("(" + x + "," + i + "," + 0 + ")");//(r,c,0)
    ...
    public int getPic(int x, int y)//(x,y) is the coordinate of butn;
    {
        Random generator = new Random();//Randow data create;
        int i = 0;

        i = generator.nextInt(7) + 1;//random integer 7 ;0,6 +1 to get 1,7 random
        ImageIcon pic = new ImageIcon("icons/" + i + ".gif");//randon imgicon created

        jewel[x][y] = new JButton();//New btn in the given coordinate
        jewel[x][y].setIcon(pic);//set icon to randomly generated img
        return i;
    }
    ...
    public boolean compHorizontal(int mode) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (samePiece(i, j, i, j + 1)) {//(x1,y1,x2,y2)
                    counter++;//increasse if hori btn have same img
                } else {
                    if (counter > 1) {
                        if (mode == 0) {
                            eliminateRow(i, j, counter, 1);//set img to blank if more than 3 
                        }
    ...
    /*
     * Test isGameOver function public void endGame() {
     *
     * for(int j=0; j < 8;j++) { for(int i=0; i < 8;i++) { switch((i+j)%3) {
     * case 0: setPic(j,i,1); break; case 1: setPic(j,i,2); break; case 2:
     * setPic(j,i,3); break; } } }
	}
     */
    public boolean samePiece(int x1, int y1, int x2, int y2) {
        if (x2 > 7 || y2 > 7) {//check wheather the last button is reached
            return false;
        }
                            //"(2,3,imgid)"                                //"(2,4,imgid)"
